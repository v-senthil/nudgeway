...
// Nudgeway metric families (HTTP, providers, workers, webhooks, Kafka,
// WebSocket). All metric names follow the convention
// `nudgeway_<subsystem>_<name>_<unit>` and are registered on a dedicated
...
// Metrics owns the Nudgeway Prometheus registry and every metric family the
...
// `/metrics` route.
type Metrics struct {
	// Registry is the Prometheus registry all Nudgeway metrics register onto.
...
// New constructs a Metrics with a fresh registry, registers every Nudgeway
...
func New() *Metrics {
	...
	m := &Metrics{
		...
		ProviderCallsTotal: prometheus.NewCounterVec(prometheus.CounterOpts{
			Name: "nudgeway_provider_calls_total",
		...
		ProviderCallDurationSeconds: prometheus.NewHistogramVec(prometheus.HistogramOpts{
			Name:    "nudgeway_provider_call_duration_seconds",
		...
		WorkerJobsTotal: prometheus.NewCounterVec(prometheus.CounterOpts{
			Name: "nudgeway_worker_jobs_total",
		...
		WorkerJobDurationSeconds: prometheus.NewHistogramVec(prometheus.HistogramOpts{
			Name:    "nudgeway_worker_job_duration_seconds",
		...
		WorkerJobRetriesTotal: prometheus.NewCounterVec(prometheus.CounterOpts{
			Name: "nudgeway_worker_job_retries_total",
		...
		WebhookEventsReceivedTotal: prometheus.NewCounterVec(prometheus.CounterOpts{
			Name: "nudgeway_webhook_events_received_total",
		...
		KafkaProducerBatchBytesTotal: prometheus.NewCounterVec(prometheus.CounterOpts{
			Name: "nudgeway_kafka_producer_batch_bytes_total",
		...
		KafkaConsumerLagRecords: prometheus.NewGaugeVec(prometheus.GaugeOpts{
			Name: "nudgeway_kafka_consumer_lag_records",
		...
		WebSocketConnections: prometheus.NewGaugeVec(prometheus.GaugeOpts{
			Name: "nudgeway_websocket_connections",
